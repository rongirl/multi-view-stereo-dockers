...
parser.add_argument(
    "--ply_path", type=str, default="outputs/[exp_name]/dypcd_fusion_plys"
)

parser.add_argument(
    "--split", type=str, default="intermediate", choices=["intermediate", "advanced"]
)
parser.add_argument(
    "--list_file", type=str, default="datasets/lists/tnt/intermediate.txt"
)
...
parser.add_argument(
    "--cam_mode", type=str, default="origin", choices=["origin", "short_range"]
)
parser.add_argument(
    "--img_mode", type=str, default="resize", choices=["resize", "crop"]
)
...
def check_geometric_consistency(
    ...
    for i in range(2, 11):
        ...
        mask = np.logical_and(
            dist < i * args.dist_base, relative_depth_diff < i * args.rel_diff_base
        )
        masks.append(mask)
...
def filter_depth(scene, root_dir, split, out_dir, plyfilename, fusion_exp):
    ...
    for ref_view, src_views in pair_data:
        ...
        if args.cam_mode == "short_range":
            ref_intrinsics, ref_extrinsics = read_camera_parameters(
                os.path.join(
                    ...
                    split,
                    scene,
                    "cams_{}/{:0>8}_cam.txt".format(scene.lower(), ref_view),
                )
            )
        elif args.cam_mode == "origin":
            ref_intrinsics, ref_extrinsics = read_camera_parameters(
                os.path.join(
                    root_dir, split, scene, "cams/{:0>8}_cam.txt".format(ref_view)
                )
            )

        ref_img = read_img(
            os.path.join(root_dir, split, scene, "images/{:0>8}.jpg".format(ref_view))
        )
        ref_depth_est = read_pfm(
            os.path.join(out_dir, scene, "depth_est/{:0>8}.pfm".format(ref_view))
        )[0]
        confidence = read_pfm(
            os.path.join(out_dir, scene, "confidence/{:0>8}.pfm".format(ref_view))
        )[0]
            ...
            if args.cam_mode == "short_range":
                src_intrinsics, src_extrinsics = read_camera_parameters(
                    os.path.join(
                        ...
                        split,
                        scene,
                        "cams_{}/{:0>8}_cam.txt".format(scene.lower(), src_view),
                    )
                )
            elif args.cam_mode == "origin":
                src_intrinsics, src_extrinsics = read_camera_parameters(
                    os.path.join(
                        root_dir, split, scene, "cams/{:0>8}_cam.txt".format(src_view)
                    )
                )
            # the estimated depth of the source view
            src_depth_est = read_pfm(
                os.path.join(out_dir, scene, "depth_est/{:0>8}.pfm".format(src_view))
            )[0]
        ...
        if ref_view < 3:
            ...
            save_mask(
                os.path.join(out_dir, scene, "mask/{:0>8}_photo.png".format(ref_view)),
                photo_mask,
            )
            save_mask(
                os.path.join(out_dir, scene, "mask/{:0>8}_geo.png".format(ref_view)),
                geo_mask,
            )
            save_mask(
                os.path.join(out_dir, scene, "mask/{:0>8}_final.png".format(ref_view)),
                final_mask,
            )

        print(
            "processing {}, ref-view{:0>2}, photo/geo/final-mask:{:.3f}/{:.3f}/{:.3f}".format(
                os.path.join(out_dir, scene),
                ref_view,
                photo_mask.mean(),
                geo_mask.mean(),
                final_mask.mean(),
...
def dypcd_filter_worker(scene):
    ...
    filter_depth(
        ...
        args.split,
        args.out_dir,
        os.path.join(args.out_dir, fusion_exp["ply_path"], save_name),
        fusion_exp,
    )
