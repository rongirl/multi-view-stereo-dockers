...
def load_data(root_path, depth_path, thresh, device):

    depths = []
    projs = []
    rgbs = []
    pair_file = "pair.txt"
    pair_path = Path(root_path) / pair_file
    with open(pair_path, "r") as f:
        num_viewpoint = int(f.readline())
        for view in range(num_viewpoint):
            img_filename = "{}/images/{:08d}.jpg".format(depth_path, view)
            cam_filename = "{}/cams/{:08d}_cam.txt".format(depth_path, view)
            depth_filename = "{}/depth_est/{:08d}.pfm".format(depth_path, view)
            confidence_filename = "{}/confidence/{:08d}.pfm".format(depth_path, view)
...
def open3d_filter(args):
    with torch.no_grad():
        mkdir_p(args.ply_path)

        depths, projs, rgbs = load_data(
            args.root_path, args.depth_path, args.prob_thresh, args.device
        )
        ...
        for i in range(tot_frame):
            pc_buff = torch.zeros(
                (3, height, width), device=depths.device, dtype=depths.dtype
            )
            val_cnt = torch.zeros(
                (1, height, width), device=depths.device, dtype=depths.dtype
            )
            j = 0
            batch_size = 20
        ...
        write_ply("{}/mvsnet.ply".format(args.ply_path), np.concatenate(points, axis=0))
        del points, depths, rgbs, projs

        gc.collect()

        print("Save {}/mvsnet.ply successful.".format(args.ply_path))
