...
def load_data(root_path, depth_path, scene_name, thresh):
    ...
    for view in range(49):
        ...
        depth_filename = "{}/{}/depth_est/{:08d}.pfm".format(
            depth_path, scene_name, view
        )
        confidence_filename = "{}/{}/confidence/{:08d}.pfm".format(
            depth_path, scene_name, view
        )
...
def open3d_filter():
    with torch.no_grad():
        ...
        for i, scene in enumerate(all_scenes):
            ...
            print(
                "{}/{} {}:".format(i, len(all_scenes), scene),
                "------------------------",
            )

            depths, projs, rgbs = load_data(
                args.root_path, args.depth_path, scene, args.prob_thresh
            )
            ...
            ply_id = int(scene[4:])
            write_ply(
                "{}/mvsnet{:03d}.ply".format(args.ply_path, ply_id),
