...
def mvsnet_to_gipuma(scan_folder, scan, root_dir, gipuma_point_folder):
    ...
    for view in range(0, 49):
        in_image_file = os.path.join(
            image_folder, "rect_{:03d}_3_r5000.png".format(view + 1)
        )  # Our image start from 1
        ...
        out_image = cv2.resize(
            in_image,
            None,
            fx=1.0 / args.downsample_factor,
            fy=1.0 / args.downsample_factor,
    ...
    for view in range(0, 49):
        ...
        sub_depth_folder = os.path.join(
            gipuma_point_folder, gipuma_prefix + "{:08d}".format(view)
        )
        if not os.path.isdir(sub_depth_folder):
            os.mkdir(sub_depth_folder)
        in_depth_pfm = os.path.join(
            depth_folder, "{:08d}_prob_filtered.pfm".format(view)
        )
...
def probability_filter(scan_folder, prob_threshold):
    ...
    for view in range(0, 49):
        init_depth_map_path = os.path.join(
            depth_folder, "{:08d}.pfm".format(view)
        )  # New dataset outputs depth start from 0.
        prob_map_path = os.path.join(
            prob_folder, "{:08d}.pfm".format(view)
        )  # Same as above
...
def collectPly(point_folder, scan_id):
    ...
    model_dir = [
        item for item in os.listdir(point_folder) if item.startswith("consistencyCheck")
    ][-1]
...
if __name__ == "__main__":
    ...
    for i, scan in enumerate(scans):
        ...
        depth_map_fusion(
            fusibile_workspace, fusibile_exe_path, disp_threshold, num_consistent
        )
