...
class Adapter(AdapterBase):
    ...
    def _load_model(self):
        ...
        model = GeoMVSNet(
            ...
            hypo_plane_num_stages=[
                int(n) for n in args.hypo_plane_num_stages.split(",")
            ],
        ...
        model.cuda()
        model.eval()
        return img_loader, model, args

    def __save_depth_maps(self, filename, depth_est, outdir):
        depth_filename = "".join(
            [outdir, filename.format(Var.depth_est, Var.format_pfm)]
        )
    ...
    def __save_images_cameras(self, filename, camera, image, outdir):
        ...
        camera = camera[0]
        cam_filename = "".join(
            [outdir, filename.format(Var.cameras, Var.cameras_format)]
        )
        img_filename = "".join(
            [outdir, filename.format(Var.images_name, Var.format_images)]
        )
    ...
    def __save_confidence_maps(
        self, filename, photometric_confidence, confidence_list, outdir, levels
    ):
        confidence_filename = "".join(
            [outdir, filename.format(Var.confidence, Var.format_pfm)]
        )
            ...
            if stage_idx != levels - 1:
                confidence_filename = "".join(
                    [
                        outdir,
                        filename.format(
                            Var.confidence, Var.stage + str(stage_idx) + Var.format_pfm
                        ),
                    ]
                )
            else:
    ...
    def _process(self):
        img_loader, model, args = self._load_model()
        with torch.no_grad():
            for batch_idx, sample in enumerate(img_loader):
                ...
                for filename, cam, img, depth_est, photometric_confidence in zip(
                    ...
                    confidence_list = [
                        outputs["stage{}".format(i)]["photometric_confidence"].squeeze(
                            0
                        )
                        for i in range(1, 5)
                    ]
