...
def geomvsnet_loss(inputs, depth_gt_ms, mask_ms, **kwargs):
    ...
    total_loss = torch.tensor(
        0.0, dtype=torch.float32, device=mask_ms["stage1"].device, requires_grad=False
    )
    pw_loss_stages = []
    dds_loss_stages = []
    for stage_idx, (stage_inputs, stage_key) in enumerate(
        ...
        # dds loss
        dds_loss = depth_distribution_similarity_loss(
            depth, depth_gt, mask, depth_min, depth_max
        )
...
def pixel_wise_loss(prob_volume, depth_gt, mask, depth_value):
    ...
    cross_entropy_image = -torch.sum(
        gt_index_volume * torch.log(prob_volume + 1e-12), dim=1
    ).squeeze(1)
...
def depth_distribution_similarity_loss(depth, depth_gt, mask, depth_min, depth_max):
    ...
    M_bins = 48
    kl_min = torch.min(torch.min(depth_gt), depth.mean() - 3.0 * depth.std())
    kl_max = torch.max(torch.max(depth_gt), depth.mean() + 3.0 * depth.std())
