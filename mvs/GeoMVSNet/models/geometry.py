...
class GeoFeatureFusion(nn.Module):
    def __init__(
        ...
        self.convolutional_layer_encoding = (
            convolutional_layer_encoding  # std / uv / z / xyz
        )
    ...
    def forward(
        ...
        rgb = rgb
        depth_min, depth_max = (
            depth_values[:, 0, None, None, None],
            depth_values[:, -1, None, None, None],
        )
        ...
        if self.convolutional_layer_encoding == "std":
            ...
            geo_s2 = self.geofeature(
                d_s2, vnorm_s2, unorm_s2, H / 2, W / 2, c352, c1216, f352, f1216
            )
            geo_s3 = self.geofeature(
                d_s3, vnorm_s3, unorm_s3, H / 4, W / 4, c352, c1216, f352, f1216
            )
            geo_s4 = self.geofeature(
                d_s4, vnorm_s4, unorm_s4, H / 8, W / 8, c352, c1216, f352, f1216
            )
        ...
        if stage_idx >= 1:
            ...
            if stage_idx == 1:
                ...
                if self.add_origin_feat_flag:
                    final_feature = self.final_decoder_stage1(
                        rgbdepth_feature + origin_feat
                    )
                else:
        ...
        if stage_idx >= 2:
            ...
            if stage_idx == 2:
                ...
                if self.add_origin_feat_flag:
                    final_feature = self.final_decoder_stage2(
                        rgbdepth_feature + origin_feat
                    )
                else:
        ...
        if stage_idx >= 3:
            ...
            if stage_idx == 3:
                ...
                if self.add_origin_feat_flag:
                    final_feature = self.final_decoder_stage3(
                        rgbdepth_feature + origin_feat
                    )
                else:
...
class GeoRegNet2d(nn.Module):
    def __init__(
        self, input_channel=128, base_channel=32, convolutional_layer_encoding="std"
    ):
        ...
        self.conv_init = ConvBnReLU3D(
            input_channel, out_channels=8, kernel_size=(1, 3, 3), pad=(0, 1, 1)
        )
...
def regconv1x1(
    in_planes, out_planes, kernel_size, stride, padding, groups=1, bias=False
):
    return nn.Conv3d(
        in_planes,
        out_planes,
