...
class GeoMVSNet(nn.Module):
    ...
    def forward(
        self, imgs, proj_matrices, intrinsics_matrices, depth_values, filename=None
    ):
        ...
        for stage_idx in range(self.levels):
            ...
            if stage_idx == 0:
                ...
                ref_img_stage = F.interpolate(
                    imgs[0],
                    size=None,
                    scale_factor=1.0 / 2 ** (3 - stage_idx),
                    mode="bilinear",
                    align_corners=False,
                )
            ...
            if stage_idx == 0:
                depth_hypo = init_inverse_range(
                    depth_values,
                    self.hypo_plane_num_stages[stage_idx],
                    img[0].device,
                    img[0].dtype,
                    H,
                    W,
            ...
            depth_est_filtered = frequency_domain_filter(
                depth_est, rho_ratio=self.curriculum_learning_rho_ratios[stage_idx]
            )
...
class StageNet(nn.Module):
    ...
    def forward(
        ...
        for src_idx, (src_fea, src_proj) in enumerate(zip(src_features, src_projs)):
            ...
            warped_src = homo_warping(
                src_fea, src_proj_new, ref_proj_new, depth_hypo
            )  # B C D H W

            warped_src = warped_src.reshape(
                B, group_cor_dim, C // group_cor_dim, D, H, W
            )
            ref_volume = ref_volume.reshape(
                B, group_cor_dim, C // group_cor_dim, D, H, W
            )
            cor_feat = (warped_src * ref_volume).mean(2)  # B G D H W
            del warped_src, src_proj, src_fea

            cor_weight = torch.softmax(cor_feat.sum(1) / self.attn_temp, 1) / math.sqrt(
                C
            )  # B D H W
