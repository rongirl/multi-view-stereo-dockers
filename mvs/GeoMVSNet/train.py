...
def train(
    model, model_loss, optimizer, TrainImgLoader, TestImgLoader, start_epoch, args
):
    if args.lr_scheduler == "MS":
        milestones = [
            len(TrainImgLoader) * int(epoch_idx)
            for epoch_idx in args.lrepochs.split(":")[0].split(",")
        ]
        ...
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=int(args.epochs * len(TrainImgLoader)), eta_min=0
        )
        ...
        lr_scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer,
            lr_lambda=lambda epoch: 0.9 ** ((epoch - 1) / len(TrainImgLoader)),
            last_epoch=len(TrainImgLoader) * start_epoch - 1,
        )
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            loss, scalar_outputs, image_outputs = train_sample(
                model, model_loss, optimizer, sample, args
            )
            lr_scheduler.step()
        ...
        # save checkpoint
        if (not is_distributed) or (dist.get_rank() == 0):
            if ((epoch_idx + 1) % args.save_freq == 0) or (
        ...
        if (epoch_idx % args.eval_freq == 0) or (epoch_idx == args.epochs - 1):
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                global_step = len(TrainImgLoader) * epoch_idx + batch_idx
                do_summary = global_step % args.summary_freq == 0
                loss, scalar_outputs, image_outputs = test_sample_depth(
                    model, model_loss, sample, args
                )
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        if not args.notensorboard:
                            tb_save_scalars(
                                tb_writer, "test", scalar_outputs, global_step
                            )
                            tb_save_images(
                                tb_writer, "test", image_outputs, global_step
                            )
                        logger.info(
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                if not args.notensorboard:
                    tb_save_scalars(
                        tb_writer, "fulltest", avg_test_scalars.mean(), global_step
                    )
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    depth_gt, mask = (
        depth_gt_ms["stage{}".format(args.levels)],
        mask_ms["stage{}".format(args.levels)],
    )
    ...
    return (
        tensor2float(scalar_outputs["loss"]),
...
if __name__ == "__main__":
    ...
    model = GeoMVSNet(
        ...
        depth_interal_ratio_stages=[
            float(ir) for ir in args.depth_interal_ratio_stages.split(",")
        ],
    ...
    if is_distributed:
        if dist.get_rank() == 0:
            logger.info(
                "Let's use {} GPUs in distributed mode!".format(
        ...
        if torch.cuda.is_available():
            logger.info(
                "Let's use {} GPUs in parallel mode.".format(torch.cuda.device_count())
            )
    ...
    if args.which_dataset == "dtu":
        ...
        train_dataset = BlendedMVSDataset(
            args.trainpath,
            args.trainlist,
            "train",
            args.n_views,
        ...
        test_dataset = BlendedMVSDataset(
            args.testpath, args.testlist, "val", args.n_views, img_wh=(768, 576)
        )
